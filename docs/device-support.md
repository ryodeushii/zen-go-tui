...
The validated Surround global contract adds a capability-gated `F3` page for authoritative 2.0 level (`0..760`, displayed as `-60.0..+16.0 dB`) and lip-sync delay (`6..45`, displayed as `0.6..4.5 ms`). Captured 2.1 state is read-only. Unknown, stale, disconnected, or missing state disables writes, and a timed-out pending write remains session-locked until a fresh controller/device session. The page intentionally omits speakers, EQ, bass management, meters, masks, and format controls. Zen Go has no validated Surround contract, so it has no Surround tab or `F3` page.
