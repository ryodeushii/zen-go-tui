...
Orion is `Supported` with `RuntimeDriverKind::Profile`. Its validated Mix-1 AuraVerb contract adds a capability-gated `F2` page with Enabled and the eight capture-backed raw `0..100` fields. The page identifies the Mixer 1 SEND FX context and intentionally omits a mix selector, presets, and send-routing controls. Authoritative or complete pending state is editable; unknown, stale, disconnected, timed-out, or missing state is read-only. Zen Go has no validated AuraVerb wire contract, so it has no AuraVerb tab or `F2` page.

The validated Surround global contract adds a capability-gated `F3` page for authoritative 2.0 level (`0..760`, displayed as `-60.0..+16.0 dB`) and lip-sync delay (`6..45`, displayed as `0.6..4.5 ms`). Captured 2.1 global state is read-only. Unknown, stale, disconnected, or missing state disables writes, and a timed-out pending write remains session-locked until a fresh controller/device session.

Orion's separate finite category-`0x1a` contract adds a strictly read-only 16-band Speaker EQ view in two banks. Fresh known 2.0 exposes only `L`/`R`; fresh known 2.1 exposes only `L`/`R`/`LFE`. Frequency, Q, signed gain, and raw mode bytes are displayed; unknown modes remain readable as raw bytes. The four leading candidate-head bytes are omitted because their dynamic level/delay/invert meaning is unverified. No opcode-`0x87` write action or capability exists. Speaker writes, presets, copy/paste, reset, bass management, meters, masks, format controls, and labels for indices 3–15 remain unavailable. Zen Go has neither validated Surround contract, so it has no Surround tab or `F3` page.
...
The current profile implements only Mix 2 strips 20–32 in the selector-gated mixer bank. A capture confirms Mix 1 strips 1–32 at @125–156 when same-report @121 equals 21. @157–188 is an exact mirror in all 38,746 reports. Root revision `dbf1314` does not implement this Mix 1 mapping. The capture contains no target-0 selector-21 write, so it does not authorize that command.

The [active mix selection contract](protocol/orion-active-mix-selection.md) confirms a separate target-1 command. Values `0..3` select UI Mix 1 through Mix 4 and read back at `0x73` @122. Full-report @121 stayed 18 during all four selections, and all @125–188 lanes stayed 96. The no-signal capture does not prove that active UI selection changes the physical meter bank. Runtime support remains unimplemented for task 54.

The profile retains six provisional one-lane output feeds. A user-approved packet-order hypothesis maps `0x73` @157/@158/@159/@160/@177/@178 to output ids 0/1/2/3/4/5. These bytes overlap Mix 1 mirrors under selector 21, so they are not unconditional output lanes. The user reports that current Monitor A and Headphone 1 meter cards display Mix 1 strips 1 and 2. This observation matches the overlap but does not prove physical-output ownership. The meter stage, physical post-fader ownership, and L/R geometry remain unknown. The split of playback-coupled @177/@178 across Reamp and Monitor B remains an ordering assumption.

Free-running `0x75` requires byte 1 `0x1f`, while byte 1 `0x00` identifies readback responses. The `0x75` @32/@48 and @33/@49 pairs are route-correlated lanes without fixed ownership. Their meter stage and calibration remain unresolved. See [the bounded Orion meter evidence](protocol/orion-meter-evidence.md).
...
Todo 53 tracks the reported missing mixer per-strip gain control. It must inspect the existing fader UI, backend support, and captures before implementation. Mixer per-strip gain is separate from physical preamp gain.
