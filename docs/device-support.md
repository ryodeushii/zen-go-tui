...
Orion is `Supported` with `RuntimeDriverKind::Profile`. The generator applies an Orion-only, non-numbered framing assumption (`transport.uses_numbered_reports: false`); descriptor and hardware validation of that assumption remains pending. Confirmed profile controls are enabled independently of metering. The profile stores the user-approved packet-order hypothesis `0x73` @157/@158/@159/@160/@177/@178 → output ids 0/1/2/3/4/5 as six provisional one-lane output feeds. This is not observed output ownership: meter stage and physical post-fader ownership remain unknown, no L/R geometry is inferred, and splitting playback-coupled @177/@178 across Reamp and Monitor B is explicitly an ordering assumption. Physical preamp meters remain unavailable. Mirror regions and the duplicate `0x75` @34/@35 candidate are excluded; free-running `0x75` requires byte 1 `0x1f`, byte 1 `0x00` readback responses are excluded, and @32 is aggregate/monitor only. See [the bounded Orion meter evidence](protocol/orion-meter-evidence.md).
...
A standalone Orion physical-input meter command or per-channel HID preamp-meter mapping is not supported by current evidence. The root runtime therefore leaves physical preamp meters unavailable. Output-card meters are visibly provisional profile assignments and must not be read as a claim about the physical post-fader stage. The bundled standalone CLI remains a separate follow-up.
